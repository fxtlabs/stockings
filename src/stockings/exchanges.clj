...
(defn- convert-record
  "Transforms a CSV record into a Company record.
   ...
   The resulting security map retains the exchange, symbol, name, IPO year,
...
(defn get-companies
  "Requests a list of the companies traded on the stock exchange denoted by
   the supplied keyword. If not keyword is provided, it returns a merged list
...
(defn build-lookup
  "Builds a lookup function corresponding to the supplied list of companies
   ...
   optionally be qualified with a stock exchange (e.g. \"NASDAQ:GOOG\").
